!function() {
  ...
  var eps = 1e-6, eps2 = eps * eps, pi = Math.PI, tau = 2 * pi, taueps = tau - eps, halfpi = pi / 2, d3v3_radians = pi / 180, d3v3_degrees = 180 / pi;
  function d3v3_sgn(x) {
  ...
  d3v3.interpolateZoom = function(p0, p1) {
    ...
    function interpolate(t) {
      var s = t * S;
      if (dr) {
        var coshr0 = d3v3_cosh(r0), u = w0 / (rho2 * d1) * (coshr0 * d3v3_tanh(rho * s + r0) - d3v3_sinh(r0));
        return [ ux0 + u * dx, uy0 + u * dy, w0 * coshr0 / d3v3_cosh(rho * s + r0) ];
      }
  ...
  var d3v3_geo_area = {
    ...
    polygonEnd: function() {
      ...
      d3v3_geo_areaSum += area < 0 ? 4 * pi + area : area;
  ...
  function d3v3_geo_areaRingStart() {
    ...
    d3v3_geo_area.point = function(lambda, phi) {
      d3v3_geo_area.point = nextPoint;
      lambda0 = (lambda00 = lambda) * d3v3_radians, cosphi0 = Math.cos(phi = (phi00 = phi) * d3v3_radians / 2 + pi / 4),
      sinphi0 = Math.sin(phi);
    };
    function nextPoint(lambda, phi) {
      lambda *= d3v3_radians;
      phi = phi * d3v3_radians / 2 + pi / 4;
  ...
  d3v3.geo.bounds = function() {
    ...
    var bound = {
      ...
      polygonEnd: function() {
        ...
        if (d3v3_geo_areaRingSum < 0) lambda0 = -(lambda1 = 180), phi0 = -(phi1 = 90); else if (dlambdaSum > eps) phi1 = 90; else if (dlambdaSum < -eps) phi0 = -90;
    ...
    function linePoint(lambda, phi) {
      var p = d3v3_geo_cartesian([ lambda * d3v3_radians, phi * d3v3_radians ]);
      if (p0) {
        ...
        var dlambda = lambda - lambda_, s = dlambda > 0 ? 1 : -1, lambdai = inflection[0] * d3v3_degrees * s, antimeridian = abs(dlambda) > 180;
        if (antimeridian ^ (s * lambda_ < lambdai && lambdai < s * lambda)) {
          var phii = inflection[1] * d3v3_degrees;
    ...
    function ringPoint(lambda, phi) {
      ...
      d3v3_geo_area.point(lambda, phi);
    ...
    function ringEnd() {
      ...
      if (abs(dlambdaSum) > eps) lambda0 = -(lambda1 = 180);
      range[0] = lambda0, range[1] = lambda1;
      p0 = null;
    }
  ...
  d3v3.geo.centroid = function(object) {
    ...
    if (m < eps2) {
      ...
      if (d3v3_geo_centroidW1 < eps) x = d3v3_geo_centroidX0, y = d3v3_geo_centroidY0, z = d3v3_geo_centroidZ0;
      m = x * x + y * y + z * z;
      if (m < eps2) return [ NaN, NaN ];
    }
  ...
  function d3v3_geo_centroidPoint(lambda, phi) {
    lambda *= d3v3_radians;
    var cosphi = Math.cos(phi *= d3v3_radians);
    d3v3_geo_centroidPointXYZ(cosphi * Math.cos(lambda), cosphi * Math.sin(lambda), Math.sin(phi));
  }
  ...
  function d3v3_geo_centroidLineStart() {
    var x0, y0, z0;
    d3v3_geo_centroid.point = function(lambda, phi) {
      lambda *= d3v3_radians;
      var cosphi = Math.cos(phi *= d3v3_radians);
    ...
    function nextPoint(lambda, phi) {
      lambda *= d3v3_radians;
      var cosphi = Math.cos(phi *= d3v3_radians), x = cosphi * Math.cos(lambda), y = cosphi * Math.sin(lambda), z = Math.sin(phi), w = Math.atan2(Math.sqrt((w = y0 * z - z0 * y) * w + (w = z0 * x - x0 * z) * w + (w = x0 * y - y0 * x) * w), x0 * x + y0 * y + z0 * z);
  ...
  function d3v3_geo_centroidRingStart() {
    ...
    function nextPoint(lambda, phi) {
      lambda *= d3v3_radians;
      var cosphi = Math.cos(phi *= d3v3_radians), x = cosphi * Math.cos(lambda), y = cosphi * Math.sin(lambda), z = Math.sin(phi), cx = y0 * z - z0 * y, cy = z0 * x - x0 * z, cz = x0 * y - y0 * x, m = Math.sqrt(cx * cx + cy * cy + cz * cz), u = x0 * x + y0 * y + z0 * z, v = m && -d3v3_acos(u) / m, w = Math.atan2(m, u);
  ...
  function d3v3_geo_clipSort(a, b) {
    return ((a = a.x)[0] < 0 ? a[1] - halfpi - eps : halfpi - a[1]) - ((b = b.x)[0] < 0 ? b[1] - halfpi - eps : halfpi - b[1]);
  }
  var d3v3_geo_clipAntimeridian = d3v3_geo_clip(d3v3_true, d3v3_geo_clipAntimeridianLine, d3v3_geo_clipAntimeridianInterpolate, [ -pi, -pi / 2 ]);
    ...
    return {
      ...
      point: function(lambda1, phi1) {
        ...
        if (abs(dlambda - pi) < eps) {
          ...
          if (abs(lambda0 - slambda0) < eps) lambda0 -= slambda0 * eps;
          if (abs(lambda1 - slambda1) < eps) lambda1 -= slambda1 * eps;
          phi0 = d3v3_geo_clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1);
          listener.point(slambda0, phi0);
          listener.lineEnd();
          listener.lineStart();
          listener.point(slambda1, phi0);
  ...
  function d3v3_geo_clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
    var cosphi0, cosphi1, sinlambda0_lambda1 = Math.sin(lambda0 - lambda1);
    return abs(sinlambda0_lambda1) > eps ? Math.atan((Math.sin(phi0) * (cosphi1 = Math.cos(phi1)) * Math.sin(lambda1) - Math.sin(phi1) * (cosphi0 = Math.cos(phi0)) * Math.sin(lambda0)) / (cosphi0 * cosphi1 * sinlambda0_lambda1)) : (phi0 + phi1) / 2;
  }
  ...
  function d3v3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        d3v3_geo_areaRingSum.add(Math.atan2(k * sdlambda * Math.sin(adlambda), cosphi0 * cosphi + k * Math.cos(adlambda)));
        ...
        if (antimeridian ^ lambda0 >= meridian ^ lambda >= meridian) {
          ...
          var phiarc = (antimeridian ^ dlambda >= 0 ? -1 : 1) * d3v3_asin(intersection[2]);
    ...
    return (polarAngle < -eps || polarAngle < eps && d3v3_geo_areaRingSum < 0) ^ winding & 1;
  }
  function d3v3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, notHemisphere = abs(cr) > eps, interpolate = d3v3_geo_circleInterpolate(radius, 6 * d3v3_radians);
    return d3v3_geo_clip(visible, clipLine, interpolate, smallRadius ? [ 0, -radius ] : [ -pi, radius - pi ]);
    ...
    function intersect(a, b, two) {
      ...
      var sigmalambda = lambda1 - lambda0, polar = abs(sigmalambda - pi) < eps, meridian = polar || sigmalambda < eps;
      if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
      if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs(q[0] - lambda0) < eps ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : sigmalambda > pi ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
  ...
  function d3v3_geo_clipExtent(x0, y0, x1, y1) {
    ...
    function corner(p, direction) {
      return abs(p[0] - x0) < eps ? direction > 0 ? 0 : 3 : abs(p[0] - x1) < eps ? direction > 0 ? 2 : 1 : abs(p[1] - y0) < eps ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
    }
  ...
  function d3v3_geo_conic(projectAt) {
    var phi0 = 0, phi1 = pi / 3, m = d3v3_geo_projectionMutator(projectAt), p = m(phi0, phi1);
  ...
  function d3v3_geo_conicEqualArea(phi0, phi1) {
    ...
    forward.invert = function(x, y) {
      var rho0_y = rho0 - y;
      return [ Math.atan2(x, rho0_y) / n, d3v3_asin((C - (x * x + rho0_y * rho0_y) * n * n) / (2 * n)) ];
    };
    return forward;
  }
  ...
  d3v3.geo.albersUsa = function() {
    ...
    albersUsa.translate = function(_) {
      ...
      alaskaPoint = alaska.translate([ x - .307 * k, y + .201 * k ]).clipExtent([ [ x - .425 * k + eps, y + .12 * k + eps ], [ x - .214 * k - eps, y + .234 * k - eps ] ]).stream(pointStream).point;
      hawaiiPoint = hawaii.translate([ x - .205 * k, y + .212 * k ]).clipExtent([ [ x - .214 * k + eps, y + .166 * k + eps ], [ x - .115 * k - eps, y + .234 * k - eps ] ]).stream(pointStream).point;
      return albersUsa;
    };
  ...
  function d3v3_geo_resample(project) {
    var sigma2 = .5, cosMinDistance = Math.cos(30 * d3v3_radians), maxDepth = 16;
    ...
    function resampleRecursive(stream) {
      ...
      function linePoint(lambda, phi) {
        var c = d3v3_geo_cartesian([ lambda, phi ]), p = project(lambda, phi);
    ...
    function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * sigma2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), phi2 = Math.asin(c /= m), lambda2 = abs(abs(c) - 1) < eps || abs(lambda0 - lambda1) < eps ? (lambda0 + lambda1) / 2 : Math.atan2(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
        if (dz * dz / d2 > sigma2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
    ...
    resample.precision = function(_) {
      if (!arguments.length) return Math.sqrt(sigma2);
      maxDepth = (sigma2 = _ * _) > 0 && 16;
      return resample;
    };
    return resample;
  }
  ...
  function d3v3_geo_projectionMutator(projectAt) {
    ...
    }), k = 150, x = 480, y = 250, lambda = 0, phi = 0, sigmalambda = 0, sigmaphi = 0, sigmagamma = 0, sigmax, sigmay, preclip = d3v3_geo_clipAntimeridian, postclip = d3v3_identity, clipAngle = null, clipExtent = null, stream;
    ...
    projection.center = function(_) {
      if (!arguments.length) return [ lambda * d3v3_degrees, phi * d3v3_degrees ];
      lambda = _[0] % 360 * d3v3_radians;
      phi = _[1] % 360 * d3v3_radians;
      return reset();
    };
    projection.rotate = function(_) {
      if (!arguments.length) return [ sigmalambda * d3v3_degrees, sigmaphi * d3v3_degrees, sigmagamma * d3v3_degrees ];
      sigmalambda = _[0] % 360 * d3v3_radians;
      sigmaphi = _[1] % 360 * d3v3_radians;
      sigmagamma = _.length > 2 ? _[2] % 360 * d3v3_radians : 0;
      return reset();
    };
    d3v3.rebind(projection, projectResample, "precision");
    function reset() {
      projectRotate = d3v3_geo_compose(rotate = d3v3_geo_rotation(sigmalambda, sigmaphi, sigmagamma), project);
  ...
  function d3v3_geo_equirectangular(lambda, phi) {
    return [ lambda, phi ];
  }
  ...
  function d3v3_geo_identityRotation(lambda, phi) {
    return [ lambda > pi ? lambda - tau : lambda < -pi ? lambda + tau : lambda, phi ];
  }
  d3v3_geo_identityRotation.invert = d3v3_geo_equirectangular;
  function d3v3_geo_rotation(sigmalambda, sigmaphi, sigmagamma) {
    return sigmalambda ? sigmaphi || sigmagamma ? d3v3_geo_compose(d3v3_geo_rotationlambda(sigmalambda), d3v3_geo_rotationphigamma(sigmaphi, sigmagamma)) : d3v3_geo_rotationlambda(sigmalambda) : sigmaphi || sigmagamma ? d3v3_geo_rotationphigamma(sigmaphi, sigmagamma) : d3v3_geo_identityRotation;
  }
  function d3v3_geo_forwardRotationlambda(sigmalambda) {
    return function(lambda, phi) {
      return lambda += sigmalambda, [ lambda > pi ? lambda - tau : lambda < -pi ? lambda + tau : lambda, phi ];
    };
  }
  function d3v3_geo_rotationlambda(sigmalambda) {
    var rotation = d3v3_geo_forwardRotationlambda(sigmalambda);
    rotation.invert = d3v3_geo_forwardRotationlambda(-sigmalambda);
    return rotation;
  }
  function d3v3_geo_rotationphigamma(sigmaphi, sigmagamma) {
    var cossigmaphi = Math.cos(sigmaphi), sinsigmaphi = Math.sin(sigmaphi), cossigmagamma = Math.cos(sigmagamma), sinsigmagamma = Math.sin(sigmagamma);
    function rotation(lambda, phi) {
      var cosphi = Math.cos(phi), x = Math.cos(lambda) * cosphi, y = Math.sin(lambda) * cosphi, z = Math.sin(phi), k = z * cossigmaphi + x * sinsigmaphi;
      return [ Math.atan2(y * cossigmagamma - k * sinsigmagamma, x * cossigmaphi - z * sinsigmaphi), d3v3_asin(k * cossigmagamma + y * sinsigmagamma) ];
    }
    rotation.invert = function(lambda, phi) {
      var cosphi = Math.cos(phi), x = Math.cos(lambda) * cosphi, y = Math.sin(lambda) * cosphi, z = Math.sin(phi), k = z * cossigmagamma - y * sinsigmagamma;
      return [ Math.atan2(y * cossigmagamma + z * sinsigmagamma, x * cossigmaphi + k * sinsigmaphi), d3v3_asin(k * cossigmaphi - x * sinsigmaphi) ];
    };
    return rotation;
  }
  ...
  function d3v3_geo_circleAngle(cr, point) {
    ...
    return ((-a[2] < 0 ? -angle : angle) + 2 * Math.PI - eps) % (2 * Math.PI);
  }
  d3v3.geo.distance = function(a, b) {
    var sigmalambda = (b[0] - a[0]) * d3v3_radians, phi0 = a[1] * d3v3_radians, phi1 = b[1] * d3v3_radians, sinsigmalambda = Math.sin(sigmalambda), cossigmalambda = Math.cos(sigmalambda), sinphi0 = Math.sin(phi0), cosphi0 = Math.cos(phi0), sinphi1 = Math.sin(phi1), cosphi1 = Math.cos(phi1), t;
    return Math.atan2(Math.sqrt((t = cosphi1 * sinsigmalambda) * t + (t = cosphi0 * sinphi1 - sinphi0 * cosphi1 * cossigmalambda) * t), sinphi0 * sinphi1 + cosphi0 * cosphi1 * cossigmalambda);
  };
  d3v3.geo.graticule = function() {
    ...
    return graticule.majorExtent([ [ -180, -90 + eps ], [ 180, 90 - eps ] ]).minorExtent([ [ -180, -80 - eps ], [ 180, 80 + eps ] ]);
  };
  function d3v3_geo_graticuleX(y0, y1, dy) {
    var y = d3v3.range(y0, y1 - eps, dy).concat(y1);
    return function(x) {
  ...
  function d3v3_geo_graticuleY(x0, x1, dx) {
    var x = d3v3.range(x0, x1 - eps, dx).concat(x1);
    return function(y) {
  ...
  function d3v3_geo_lengthLineStart() {
    ...
    d3v3_geo_length.point = function(lambda, phi) {
      lambda0 = lambda * d3v3_radians, sinphi0 = Math.sin(phi *= d3v3_radians), cosphi0 = Math.cos(phi);
    ...
    function nextPoint(lambda, phi) {
      var sinphi = Math.sin(phi *= d3v3_radians), cosphi = Math.cos(phi), t = abs((lambda *= d3v3_radians) - lambda0), cossigmalambda = Math.cos(t);
      d3v3_geo_lengthSum += Math.atan2(Math.sqrt((t = cosphi * Math.sin(t)) * t + (t = cosphi0 * sinphi - sinphi0 * cosphi * cossigmalambda) * t), sinphi0 * sinphi + cosphi0 * cosphi * cossigmalambda);
  ...
  var d3v3_geo_azimuthalEqualArea = d3v3_geo_azimuthal(function(coslambdacosphi) {
  ...
  var d3v3_geo_azimuthalEquidistant = d3v3_geo_azimuthal(function(coslambdacosphi) {
  ...
  function d3v3_geo_conicConformal(phi0, phi1) {
    ...
    function forward(lambda, phi) {
      if (F > 0) {
        if (phi < -halfpi + eps) phi = -halfpi + eps;
      } else {
        if (phi > halfpi - eps) phi = halfpi - eps;
      }
    ...
    forward.invert = function(x, y) {
      var rho0_y = F - y, rho = d3v3_sgn(n) * Math.sqrt(x * x + rho0_y * rho0_y);
  ...
  function d3v3_geo_conicEquidistant(phi0, phi1) {
    ...
    forward.invert = function(x, y) {
      var rho0_y = G - y;
      return [ Math.atan2(x, rho0_y) / n, G - d3v3_sgn(n) * Math.sqrt(x * x + rho0_y * rho0_y) ];
    };
    return forward;
  }
  ...
  var d3v3_geo_stereographic = d3v3_geo_azimuthal(function(coslambdacosphi) {
  ...
  function d3v3_geo_transverseMercator(lambda, phi) {
  ...
  function d3v3_geom_voronoiRemoveBeach(beach) {
    ...
    var lArc = previous;
    while (lArc.circle && abs(x - lArc.circle.x) < eps && abs(y - lArc.circle.cy) < eps) {
      previous = lArc.P;
    ...
    var rArc = next;
    while (rArc.circle && abs(x - rArc.circle.x) < eps && abs(y - rArc.circle.cy) < eps) {
      next = rArc.N;
  ...
  function d3v3_geom_voronoiCloseCells(extent) {
    ...
    while (iCell--) {
      ...
      while (iHalfEdge < nHalfEdges) {
        ...
        if (abs(x3 - x2) > eps || abs(y3 - y2) > eps) {
          halfEdges.splice(iHalfEdge, 0, new d3v3_geom_voronoiHalfEdge(d3v3_geom_voronoiCreateBorderEdge(cell.site, end, abs(x3 - x0) < eps && y1 - y3 > eps ? {
            x: x0,
            y: abs(x2 - x0) < eps ? y2 : y1
          } : abs(y3 - y1) < eps && x1 - x3 > eps ? {
            x: abs(y2 - y1) < eps ? x2 : x1,
            y: y1
          } : abs(x3 - x1) < eps && y3 - y0 > eps ? {
            x: x1,
            y: abs(x2 - x1) < eps ? y2 : y0
          } : abs(y3 - y0) < eps && x3 - x0 > eps ? {
            x: abs(y2 - y0) < eps ? x2 : x0,
            y: y0
          } : null), cell.site, null));
          ++nHalfEdges;
        }
  ...
  function d3v3_geom_voronoiClipEdges(extent) {
    ...
    while (i--) {
      e = edges[i];
      if (!d3v3_geom_voronoiConnectEdge(e, extent) || !clip(e) || abs(e.a.x - e.b.x) < eps && abs(e.a.y - e.b.y) < eps) {
        e.a = e.b = null;
        edges.splice(i, 1);
      }
    }
  ...
  d3v3.geom.voronoi = function(points) {
    ...
    function sites(data) {
      return data.map(function(d, i) {
        return {
          x: Math.round(fx(d, i) / eps) * eps,
          y: Math.round(fy(d, i) / eps) * eps,
          i: i
        };
      });
    }
  ...
  d3v3.layout.pie = function() {
    var value = Number, sort = d3v3_layout_pieSortByValue, startAngle = 0, endAngle = tau, padAngle = 0;
    function pie(data) {
  ...
  d3v3.svg.arc = function() {
    ...
    function arc() {
      ...
      if (da >= taueps) return circleSegment(r1, cw) + (r0 ? circleSegment(r0, 1 - cw) : "") + "Z";
  ...
  d3v3.svg.axis = function() {
    ...
    function axis(g) {
      g.each(function() {
        ...
        var ticks = tickValues == null ? scale1.ticks ? scale1.ticks.apply(scale1, tickArguments_) : scale1.domain() : tickValues, tickFormat = tickFormat_ == null ? scale1.tickFormat ? scale1.tickFormat.apply(scale1, tickArguments_) : d3v3_identity : tickFormat_, tick = g.selectAll(".tick").data(ticks, scale1), tickEnter = tick.enter().insert("g", ".domain").attr("class", "tick").style("opacity", eps), tickExit = d3v3.transition(tick.exit()).style("opacity", eps).remove(), tickUpdate = d3v3.transition(tick.order()).style("opacity", 1), tickSpacing = Math.max(innerTickSize, 0) + tickPadding, tickTransform;
